// Définition des fonctions de la classe Image
...
void skip_comments(istream& is)
{
	...
	// Définition des fonctions de la classe Color
	...
	{
		string c;  //pas de zéro terminal faire attention
		is >> c;

		if(c != "Maison2")
			throw runtime_error("Pas un Maison2 !");
		...
		is >> height >> width;
		is.get();
		//skip_comments(is);		commentaire après l'en-tête
		cout << c;
		skip_line(is);

		uint8_t r, g, b;
		is >> g >> b >> r;
		//Color Maison2_color = new Color(r,g,b);		//les couleurs des composantes de chaque pixel


		uint16_t maxpixel;
		is >> maxpixel;
		if(maxpixel != 255)
			throw runtime_error("Precision max depassee");
	...
	// void ColorImage::writeMaison2(ostream& os) const {
  //
	// 	os << "Maison2\n"
	// 	   << "#image sauvee par D.Clement\n"
	// 	   << heigth << " " << width << "\n"
  ...
  //
	...
	void ColorImage::writeTGA(ostream& os, bool rle) const
	{cout << " ";}
	...
	{
		ColorImage * picture      = new ColorImage( getWidth(), getHeight() );
		uint16_t first_halfWidth  = getWidth() / 2;															//longueur de la première moitié de l'image
		uint16_t second_halfWidth = getWidth() - first_halfWidth;							//longueur de la seconde moitié de l'image

		for(uint16_t _y = 0; _y < first_halfWidth; _y++)
			for(uint16_t _x = 0; _x < getHeight(); _x++)
			{
				picture -> pixel(_x, _y) = pixel(_x, _y);														//affectation des couleurs de l'image courrante à la nouvelle image
			}

		for(uint16_t _y = first_halfWidth; _y < second_halfWidth; _y++)
			for(uint16_t _x = 0; _x < getHeight(); _x++ )
			{
				picture -> pixel(_x, _y) = pixel(_x, _y);
			}
