...
Image * Image::readPGM(istream& is)
{
	string c;
	is >> c;
...
void Image::clear(uint8_t color)
{
...
void Image::rectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t color)
{
...
void Image::fillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t color)
{
	if(w >= getWidth() || h >= getHeight())
...
Image * Image::simpleScale(uint16_t w, uint16_t h) const
{
  Image * picture = new Image(w, h);
...
Image * Image::bilinearScale(uint16_t w, uint16_t h) const
{
	Image * picture = new Image(w, h);
